...
#region Default Instance
		...
		public void bttnReserve_Click(System.Object sender, System.EventArgs e)
		{
			...
			{
				string a = System.Convert.ToString(Interaction.MsgBox("Подтвердить бронь?", (int) Constants.vbQuestion + Constants.vbYesNo, "Бронь"));
				if (a == Constants.vbYes.ToString())
				{
					...
					Interaction.MsgBox("Успешно забронировано!", Constants.vbInformation, "Бронь");
					clear_text();
					Module1.con.Close();
					transID();
					display_reserve();
		...
		public void cboDiscount_TextChanged(object sender, System.EventArgs e)
		{
			...
			//lblAdvancePayment.Text = "Минимальный аваносовый платеж " + (Conversion.Val(txtSubTotal.Text) * 0.5);
			Module1.rs.Dispose();
			Module1.con.Close();
		}
		...
		{
			string check_in = System.Convert.ToString(Interaction.MsgBox("Заселить гостя?", (int) Constants.vbQuestion + Constants.vbYesNo, "Заселение"));
			if (check_in == Constants.vbYes.ToString())
			{
				...
				Module1.con.Close();
				display_reserve();
				Interaction.MsgBox("Гость заселен!", Constants.vbInformation, "Заселение");
			}
		}
		
		public void bttnCancelReserve_Click(System.Object sender, System.EventArgs e)
		{
			string check_in = System.Convert.ToString(Interaction.MsgBox("Отменить бронь?", (int) Constants.vbQuestion + Constants.vbYesNo, "Отмена"));
			if (check_in == Constants.vbYes.ToString())
			{
				Module1.con.Open();
				...
				Module1.con.Close();
				display_reserve();
				Interaction.MsgBox("Бронь отменена!", Constants.vbInformation, "Отмена");
			}
		}
