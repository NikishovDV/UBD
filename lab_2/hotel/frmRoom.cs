...
#region Default Instance
		...
		public void bttnSave_Click(System.Object sender, System.EventArgs e)
		{
			...
			else
			{
				if (bttnSave.Text == "&Сохранить")
				{
					...
					try
					{
						if (dr.Read() == false)
						{
							...
							add_room.Dispose();
							Interaction.MsgBox("Номер добавлен!", Constants.vbInformation, "Внимание");
							txtID.Clear();
							txtRoomType.Clear();
							txtRoomRate.Clear();
							txtNoOfOccupancy.Clear();
						...
						{
							Interaction.MsgBox("Номер существует!", Constants.vbExclamation, "Внимание");
						}
					}
					catch (Exception ex)
					{
				...
				else
				{
					...
					update_room.Dispose();
					Interaction.MsgBox("Номер сохранить!", Constants.vbInformation, "Внимание");
					bttnSave.Text = "&Сохранить";
					txtID.Clear();
					txtRoomType.Clear();
					txtRoomRate.Clear();
		...
		{
			string a = System.Convert.ToString(Interaction.MsgBox("Обновить выбранный номер?", (int) Constants.vbQuestion + Constants.vbYesNo, "Обновление номера"));
